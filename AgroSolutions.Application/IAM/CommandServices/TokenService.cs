...
public class TokenService :ITokenService
{
    ...
    public string GenerateAnonymousToken()
    {
        ...
        var tokenDescriptor = new SecurityTokenDescriptor
        {
            Subject = new ClaimsIdentity(new Claim[]{}), // No claims for anonymous token
            Expires = DateTime.UtcNow.AddHours(4),
            SigningCredentials =
                new SigningCredentials(new SymmetricSecurityKey(key), SecurityAlgorithms.HmacSha256Signature)
        };
    ...
    public async Task<int?> ValidateToken(string token)
    {
        ...
        try
        {
            ...
            // Check if the token has any claims
            if (jwtToken.Claims.Any())
            {
                var userId = int.Parse(jwtToken.Claims.First(claim => claim.Type == ClaimTypes.Sid).Value);
                return userId;
            }
            // If the token has no claims, it's an anonymous token
            else
            {
                // Return a specific value to indicate an anonymous user, for example, -1
                return -1;
            }
        }
        catch (Exception e)
